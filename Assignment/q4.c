...
void frontBackSplitLinkedList(LinkedList *ll, LinkedList *resultFrontList, LinkedList *resultBackList)
{
    ...
    int frontlistsize = 0, backlistsize = 0;

    if ((ll->size) % 2 == 0) {
        frontlistsize = backlistsize = (ll->size) / 2;
    }

    else {
        backlistsize = (ll->size) / 2;
        frontlistsize = backlistsize + 1;
    }

    while (resultFrontList->size < frontlistsize) {
        insertNode(resultFrontList, resultFrontList->size, cur -> item);
        cur = cur -> next;
    }

    while (resultBackList->size < backlistsize) {
